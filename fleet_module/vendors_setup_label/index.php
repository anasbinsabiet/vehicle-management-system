...
<div class="row mb-2">
  <div class="col-sm-6">
    <h1 class="m-0 text-dark">Vendors Setup</h1>
  </div><!-- /.col -->
  <div class="col-sm-6">
    <ol class="breadcrumb float-sm-right">
      <li class="breadcrumb-item active"><a
          href="<?php echo $_SESSION['base_url'] . 'dashboard/index'; ?>">Dashboard</a></li>
      <li class="breadcrumb-item active">Vendors Label</li>
    </ol>
  </div><!-- /.col -->
</div><!-- /.row -->
</div><!-- /.container-fluid -->
  ...
  <div class="container-fluid">
    ...
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-lg-9">
            <h3 class="card-title">Vendors Label</h3>
          </div>
          <div class="col-lg-3">
            <a href="add.php">
      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
          <?php
          $query = $connect->prepare("SELECT * FROM `vendors_labels`");
          $query->execute();
          ...
          foreach ($result as $row) {
            ...
            ?>
            <tr>
              <td><?php echo $row['labels_id']; ?></td>
              <td><?php echo $row['labels']; ?></td>
              <td><?php echo $status; ?></td>
              <td>
                <a
                  href="add.php?id=<?php echo $row['labels_id']; ?>">
                  <button class="badge badge-info btn-xs">Edit</button>
                </a>

                <button type="button" name="delete" id="<?php echo $row["labels_id"] ?>"
    ...
    $(document).on('click', '.delete', function () {
      ...
      if (confirm("Are you sure you want to change status?")) {
        $.ajax({
          url: "action.php",
          method: "POST",
          data: {labels_id: labels_id, status: status, btn_action: btn_action},
