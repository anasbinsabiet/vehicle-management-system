...
<div class="row mb-2">
  ...
  <div class="col-sm-6">
    <ol class="breadcrumb float-sm-right">
      ...
      <li class="breadcrumb-item active">Vehicle Make</li>
    </ol>
  </div><!-- /.col -->
</div><!-- /.row -->
</div><!-- /.container-fluid -->
    ...
    <div class="card">
      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
          <?php
          $query = $connect->prepare("SELECT * FROM `vehicle_details_make`");
          $query->execute();
          ...
          foreach ($result as $row) {
            ...
            <tr>
              ...
              <td>
                <a
                  href="add.php?id=<?php echo $row['make_id']; ?>">
                  <button class="badge badge-info btn-xs">Edit</button>
                </a>

                <button type="button" name="delete" id="<?php echo $row["make_id"] ?>"
    ...
    $(document).on('click', '.delete', function () {
      ...
      if (confirm("Are you sure you want to change status?")) {
        $.ajax({
          url: "action.php",
          method: "POST",
          data: {make_id: make_id, status: status, btn_action: btn_action},
